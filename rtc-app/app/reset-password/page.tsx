...
export default function ResetPasswordPage() {
    ...
    useEffect(() => {
        // Récupérer le token depuis l'URL (peut être dans les query params ou dans le hash)
        let accessToken: string | null = null;
        let typeParam: string | null = null;

        // Essayer d'abord les query parameters (?access_token=...)
        const searchParams = new URLSearchParams(window.location.search);
        accessToken = searchParams.get("access_token");
        typeParam = searchParams.get("type");

        // Si pas trouvé, essayer le hash fragment (#access_token=...)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Appeler l'API Next.js qui proxy vers le backend Rust
            ...
            if (response.ok) {
                ...
                setError(text || "Erreur lors de la réinitialisation du mot de passe.");
            }
        } catch (err) {
    ...
    if (success) {
        return (
            <div className="flex min-h-screen items-center justify-center bg-zinc-950">
                <div className="w-full max-w-md p-8 shadow-lg shadow-orange-600/50 rounded-2xl border border-orange-500">
                    <img 
                        src="/logo_fluxy.png" 
                        alt="Logo" 
                        className="mx-auto mb-6 object-contain scale-125"
                        width={100}
                        height={100}
                    />
        ...
        <div className="flex min-h-screen items-center justify-center bg-zinc-950">
            <div className="w-full max-w-md p-8 shadow-lg shadow-orange-600/50 rounded-2xl border border-orange-500 transition-all duration-300 ease-out hover:shadow-2xl hover:shadow-orange-600/70 hover:-translate-y-2">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        <label className="block text-sm font-medium text-orange-600 mb-1">Nouveau mot de passe</label>
